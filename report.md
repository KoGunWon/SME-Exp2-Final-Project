...
딥러닝 모델이 좌표 평면 전체를 탐색하는 부담을 줄이기 위해, 측정 거리의 역수를 가중치($w_i$)로 사용하는 초기 위치($\hat{p}_{init}$)를 산출한다.$$w_i = \frac{1}{\hat{d}_i + \epsilon}, \quad \hat{p}_{init} = \frac{\sum_{i=1}^{18} w_i \cdot p_{bs,i}}{\sum_{i=1}^{18} w_i}$$이 수식은 근거리 신호일수록 정확도가 높다는 물리적 자명성을 모델의 기초 판단 근거로 주입하는 역할을 한다.
